...
### 3. 한글 텍스트 규칙 (KOREAN TEXT RULES)
```
=== KOREAN TEXT RENDERING ===
- 모든 한글은 Pretendard Bold 폰트 (weight 700+)
- 본문 텍스트 최소 28pt, 헤드라인 48pt 이상
- 한 문장은 15자 이내로 짧게 (긴 문장은 뭉개짐 유발)
- 숫자 + 키워드 조합이 긴 문장보다 렌더링 정확도 높음
- 리뷰/후기 텍스트는 핵심 한 줄만 (3줄 이상 금지)
- Q&A 답변은 10자 이내 간결하게
- px 단위 수치(14px, 24px 등) 절대 금지 — Gemini가 이미지에 렌더링함
- CSS 스펙 값(border-radius, rgba, shadow) 금지
- 텍스트 크기는 "large", "medium", "standard" 등 상대적 표현만 사용
```

### 4. 체크리스트 (모든 프롬프트 끝에 추가)
```
=== FINAL CHECKLIST ===

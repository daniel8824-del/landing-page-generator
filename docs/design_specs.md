...
### 한글 텍스트 규칙 (KOREAN TEXT RULES)
- **폰트: Pretendard Bold (weight 700+)** — 모든 한글에 필수 적용
- 본문 최소 28pt, 헤드라인 48pt 이상
- 한 문장 15자 이내 (긴 문장은 Gemini가 뭉갬)
- **px 단위/CSS 스펙 프롬프트에 절대 넣지 말 것** — Gemini가 이미지에 렌더링함
- 크기는 "large", "medium", "standard" 등 상대적 표현만 사용
...
> ⚠️ **아래 프리셋의 px/CSS 값은 설계 참고용입니다. Gemini 프롬프트에는 절대 px 수치나 CSS 스펙을 직접 넣지 마세요. "large", "medium", "rounded" 등 상대적 표현으로 변환하여 사용하세요.**
...
최종 출력:
final_detail_page.png
final_detail_page.pdf
preview.png
